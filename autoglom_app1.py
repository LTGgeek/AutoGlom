...
def load_images_from_folder(folder):
    images = []
    skip_files = {"vs_histogram.png", "glomerular_volumes.csv", "results.json"}
...
def display_json_results(data, frame):
    ...
    if bdot_file:
        ...
        Results:
            Kidney Volume:        {data['kidney_volume']:.4f} mm³
            Medulla Volume:       {data['medulla_volume']:.4f} mm³
            Artifacts Volume:     {data['blackdot_volume']:.4f} mm³
            Number of Glomeruli:  {data['n_glom']}
            Mean Glomerular  Vol.:  {data['mean_vs']:.3e} mm³
            Median Glomerular  Vol.:{data['med_vs']:.3e} mm³
            Magnitude Threshold:  {data['inten_thre']:.2f}
...
def run_script_7():
    ...
    inten_thre_entry = create_labeled_entry(
        ...
        "Magnitude Threshold:",
        "0.10",
        "The threshold of magnitude for DoG method to smooth. A higher Magnitude Threshold results in a lower number of gloms being segmented. (0-1, default: 0.10)"
    )

    unet_mask_threshold_entry = create_labeled_entry(
        script7_frame,
        "U-Net mask threshold:",
        "0.5",
    ...
    ToolTip(execute_button, "Run glomerular detection and counting analysis")
    ...
    def process():
        try:
            ...
            if algorithm == "UHDoG":
                ...
                hist_path = os.path.join(target_folderdir, f"{sector}_results", "vs_histogram.png")
                if hasattr(create_viewer_window, 'window'):
                    create_viewer_window.window.after(0, lambda: show_histogram_popup(hist_path))
...
def create_viewer_window():
    ...
    if not hasattr(create_viewer_window, 'window'):
        ...
        buttons = [
            ...
            ("Glomerular Analysis", lambda: run_script_7())
        ]
...
def update_images(new_image_list_1=[], new_image_list_2=[], new_image_list_3=[], 
    ...
    start_padding = int(sslice) if sslice is not None else 0
    ...
    if max_length != 0:
        for i, image_list in enumerate([new_image_list_1, new_image_list_2, new_image_list_3]):
            if image_list is not None and len(image_list) > 0 and len(image_list) < max_length:
                if i == 0:
                    new_image_list_1 = [blank_image] * start_padding + image_list
                elif i == 1:
                    new_image_list_2 = [blank_image] * start_padding + image_list
                elif i == 2:
                    new_image_list_3 = [blank_image] * start_padding + image_list
...
def show_histogram_popup(hist_path):
    """
    Show the saved glomerular volume histogram in a popup window.
    """
    if not os.path.exists(hist_path):
        messagebox.showwarning("Histogram not found", f"Histogram file not found:\n{hist_path}")
        return

    popup = tk.Toplevel(create_viewer_window.window)
    popup.title("Glomerular Volume Histogram")
    popup.geometry("850x700")

    img = Image.open(hist_path)

    # Resize only if the image is too large for the popup
    ...
    label.image = img_tk  # Keep a reference so the image does not disappear
